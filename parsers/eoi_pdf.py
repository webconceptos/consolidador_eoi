...
from utils.experience import total_days
...
def _extract_name_parts_old(text: str) -> dict:
    ...
    if not nombres:
        ...
        if m2:
            ...
            mword = re.search(r"\b(?!Lugar\b|Documento\b|Apellido\b|Identidad\b|Celular\b|email\b|Correo\b)([A-Za-zÁÉÍÓÚÑáéíóúñ]{2,})\b", tail, re.IGNORECASE)
            if mword:
                nombres = clean(mword.group(1))
...
def parse_eoi_pdf(pdf_path: Path, use_ocr: bool = False, debug: bool = False) -> dict:
    ...
    # Si el texto es muy corto, probablemente sea escaneado.
    # (En Alex NO aplica; el texto es largo y estructurado.)
    if len(text) < 400 and use_ocr:
        # aquí entrarías a OCR si lo implementas; por ahora devolvemos señal
        return {"needs_ocr": True, "source_file": str(pdf_path)}
    ...
    exp_general_intervals = [{"fi": fi, "ff": ff, "cargo": "", "entidad": ""} for fi, ff in gen_pairs]
    exp_esp_intervals = [{"fi": fi, "ff": ff, "cargo": "", "entidad": ""} for fi, ff in esp_pairs]

    # Días efectivos sin solapamiento (tu util)
# Convierte a date para total_days()
    gen_pairs_date = []
    for fi, ff in gen_pairs:
        d1 = _parse_date_any(fi)
        d2 = _parse_date_any(ff)
        if d1 and d2:
            gen_pairs_date.append((d1, d2))

    esp_pairs_date = []
    for fi, ff in esp_pairs:
        d1 = _parse_date_any(fi)
        d2 = _parse_date_any(ff)
        if d1 and d2:
            esp_pairs_date.append((d1, d2))

    gen_days = total_days(gen_pairs_date)
    esp_days = total_days(esp_pairs_date)
    ...
    return {
        ...
        **edu,
        "cursos": cursos_uniq,
        "experiencias": exp_general_intervals,  # si quieres mezclar general+específica, lo ajustamos
