# tasks/task_42_eval_postulantes_openai.py
# -*- coding: utf-8 -*-
...
def get_ec_blocks_from_postulante(p: dict) -> list[dict]:
    """
    Prioridad:
    p["estudios_complementarios"]["blocks"]  (estructura moderna)
    """
...
def get_ec_fallback_text(p: dict) -> str:
    """
    Fallback por si no existe estructura blocks.
    """
...
def get_formacion_text(p: dict) -> str:
    """
    Arma texto de formación académica sin romper.
    """
    # 1) Si ya existe resumen directo
    v = p.get("formacion_resumen")
    if isinstance(v, str) and v.strip():
        return v.strip()

    # 2) Si existe formacion_obligatoria como dict
    ...
    if isinstance(items, list) and items:
        ...
        for it in items:
            if not isinstance(it, dict):
                continue
            grado = str(it.get("grado", "") or "").strip()
            carrera = str(it.get("carrera", "") or "").strip()
            entidad = str(it.get("entidad", "") or "").strip()
...
def main():
    ...
    ap.add_argument("--postulantes", required=True, help="Ruta a parsed_postulantes.jsonl / consolidado.jsonl")
    ...
    for p in postulantes:
        ...
        # ---------------- FA ----------------
        print(f"[task_41] Evaluando FA => {nombre} | DNI={dni}")
        ...
        result = {
            ...
            "_meta": {
                ...
                "task": "task_41_eval_postulantes_openai",
                "evaluated_at": ts(),
            },
        }
    ...
    print(f"[task_41] ✅ Evaluación FA + EC completada")
